...
void main() {
    ...
    for(int i = 0; i < SSAO_KERNEL_SIZE; i++) {		
        ...
        samplePos = fragPos + samplePos * pc.ssaoRadius; 
        
        // project
        vec4 offset = vec4(samplePos, 1.0f);
        offset = pc.projection * offset; 
        offset.xyz /= offset.w; 
        ...
        float rangeCheck = smoothstep(0.0f, 1.0f, pc.ssaoRadius / abs(fragPos.z - sampleDepth));
        occlusion += (sampleDepth >= samplePos.z + pc.ssaoBias ? 1.0f : 0.0f) * rangeCheck;           
    }
