...
VulkanDeviceMemory::VulkanDeviceMemory(std::shared_ptr<VulkanGraphicsDevice> dev, VkDeviceMemory mem, VkMemoryType t, VkDeviceSize s)
    : gdevice(dev)
    , memory(mem)
    , type(t)
    , length(s)
...
bool VulkanDeviceMemory::invalidate(uint64_t offset, uint64_t size) {
    FVASSERT_DEBUG(memory != VK_NULL_HANDLE);
...
bool VulkanDeviceMemory::flush(uint64_t offset, uint64_t size) {
    FVASSERT_DEBUG(memory != VK_NULL_HANDLE);
