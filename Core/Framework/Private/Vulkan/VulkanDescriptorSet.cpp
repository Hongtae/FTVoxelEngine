...
VulkanDescriptorSet::VulkanDescriptorSet(std::shared_ptr<VulkanGraphicsDevice> dev,
...
void VulkanDescriptorSet::collectImageViewLayouts(ImageLayoutMap& imageLayouts, ImageViewLayoutMap& viewLayouts) {
    std::map<VkImageView, VulkanImageView*> imageViewMap;
    ...
    for (Binding& binding : bindings) {
        ...
        if (imageInfo && imageInfo->imageView != VK_NULL_HANDLE) {
            ...
            if (p != imageViewMap.end()) {
                VulkanImageView* imageView = p->second;
                FVASSERT_DEBUG(imageView->imageView == imageInfo->imageView);

                VulkanImage* image = imageView->image.get();
...
void VulkanDescriptorSet::updateImageViewLayouts(const ImageViewLayoutMap& imageLayouts) {
