...
std::weak_ptr<Logger> vulkanDebugLogger;
...
std::shared_ptr<VulkanInstance> VulkanInstance::makeInstance(
    ...
    if (gpuCount > 0) {
        ...
        for (auto& device : physicalDevices) {
            VulkanPhysicalDeviceDescription pd(device);
...
std::shared_ptr<GraphicsDevice> VulkanInstance::makeDevice(
    ...
    if (auto iter = std::find_if(
        ...
        try {
            return std::make_shared<VulkanGraphicsDevice>(shared_from_this(),
                                                    device,
                                                    requiredExtensions,
                                                    optionalExtensions);
