...
VulkanShaderBindingSet::VulkanShaderBindingSet(std::shared_ptr<VulkanGraphicsDevice> dev,
...
VulkanShaderBindingSet::~VulkanShaderBindingSet() {
    vkDestroyDescriptorSetLayout(gdevice->device, descriptorSetLayout, gdevice->allocationCallbacks());
}

std::shared_ptr<VulkanDescriptorSet> VulkanShaderBindingSet::makeDescriptorSet() const {
    std::shared_ptr<VulkanDescriptorSet> descriptorSet = gdevice->makeDescriptorSet(descriptorSetLayout, poolID);
...
VulkanShaderBindingSet::DescriptorBinding* VulkanShaderBindingSet::findDescriptorBinding(uint32_t binding) {
...
void VulkanShaderBindingSet::setBuffer(uint32_t binding, std::shared_ptr<GPUBuffer> bufferObject, uint64_t offset, uint64_t length) {
    BufferInfo bufferInfo = { bufferObject, offset, length };
    return setBufferArray(binding, 1, &bufferInfo);
}

void VulkanShaderBindingSet::setBufferArray(uint32_t binding, uint32_t numBuffers, BufferInfo* bufferArray) {
    ...
    if (descriptorBinding) {
        ...
        case VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER:
            ...
            for (uint32_t i = 0; i < availableItems; ++i) {
                auto bufferView = std::dynamic_pointer_cast<VulkanBufferView>(bufferArray[i].buffer);
...
void VulkanShaderBindingSet::setTexture(uint32_t binding, std::shared_ptr<Texture> textureObject) {
    return setTextureArray(binding, 1, &textureObject);
}

void VulkanShaderBindingSet::setTextureArray(uint32_t binding, uint32_t numTextures, std::shared_ptr<Texture>* textureArray) {
    DescriptorBinding* descriptorBinding = findDescriptorBinding(binding);
    if (descriptorBinding) {
        ...
        case VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT:
            ...
            for (uint32_t i = 0; i < availableItems; ++i) {
                ...
                auto imageView = std::dynamic_pointer_cast<VulkanImageView>(textureArray[i]);
...
void VulkanShaderBindingSet::setSamplerState(uint32_t binding, std::shared_ptr<SamplerState> samplerState) {
    return setSamplerStateArray(binding, 1, &samplerState);
}

void VulkanShaderBindingSet::setSamplerStateArray(uint32_t binding, uint32_t numSamplers, std::shared_ptr<SamplerState>* samplerArray) {
    DescriptorBinding* descriptorBinding = findDescriptorBinding(binding);
    if (descriptorBinding) {
        //descriptorBinding->descriptorWrites.clear();
        descriptorBinding->bufferInfos.clear();
        ...
        case VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER:
            ...
            for (uint32_t i = 0; i < availableItems; ++i) {
                ...
                auto sampler = std::dynamic_pointer_cast<VulkanSampler>(samplerArray[i]);
