...
SwapChain::SwapChain(std::shared_ptr<CommandQueue> q, std::shared_ptr<FV::Window> w)
    : cqueue(q)
    , window(w)
    , surface(nullptr)
    , swapchain(nullptr)
...
bool SwapChain::updateDevice() {
    ...
    for (VkImage image : swapchainImages) {
        ...
        std::shared_ptr<Image> swapChainImage = std::make_shared<Image>(gdevice, image);
...
void SwapChain::setPixelFormat(PixelFormat pf) {
    std::scoped_lock guard(lock);
    VkFormat format = getPixelFormat(pf);

    if (format != this->surfaceFormat.format) {
        if (FV::isColorFormat(pf)) {
...
FV::PixelFormat SwapChain::pixelFormat() const {
    std::scoped_lock guard(lock);
    return getPixelFormat(this->surfaceFormat.format);
}

FV::RenderPassDescriptor SwapChain::currentRenderPassDescriptor() {
...
bool SwapChain::present(FV::GPUEvent** waitEvents, size_t numEvents) {
    ...
    for (size_t i = 0; i < numEvents; ++i) {
        FV::GPUEvent* event = waitEvents[i];
        Semaphore* s = dynamic_cast<Semaphore*>(event);
        FVASSERT_DEBUG(s);
...
void SwapChain::onWindowEvent(const FV::Window::WindowEvent& e) {
    if (e.type == FV::Window::WindowEvent::WindowResized) {
        std::scoped_lock guard(lock);
        this->deviceReset = true;
    }
}
