...
namespace FV
{
	...
	public:
		...
		virtual void setRenderPipelineState(RenderPipelineState* state) = 0;
		...
		virtual void drawIndexed(uint32_t numIndices, IndexType indexType, std::shared_ptr<GPUBuffer> indexBuffer, uint32_t indexBufferOffset, uint32_t numInstances, int32_t baseVertex, uint32_t baseInstance) = 0;
	};
}
